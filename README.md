...
The published coordinates are `com.ondewo:ondewo-t2s-client-java`. Maven:

```xml
<dependency>
  <groupId>com.ondewo</groupId>
  <artifactId>ondewo-t2s-client-java</artifactId>
...
```groovy
dependencies { implementation 'com.ondewo:ondewo-t2s-client-java:VERSION' }
```
...
> [search.maven.org](https://search.maven.org/artifact/com.ondewo/ondewo-t2s-client-java).
...
dependencies { implementation 'com.github.ondewo:ondewo-t2s-client-java:VERSION' }
```

### Jar from the GitHub release

Every [release](https://github.com/ondewo/ondewo-t2s-client-java/releases) carries the built
`ondewo-t2s-client-java-<version>.jar` and its sources jar as assets. Install one into
...
mvn install:install-file \
  -Dfile=ondewo-t2s-client-java-<version>.jar \
  -DpomFile=pom.xml
```
...
```bash
git clone --recurse-submodules https://github.com/ondewo/ondewo-t2s-client-java.git
cd ondewo-t2s-client-java
...
service stubs of this client live in `ondewo.t2s`. Each proto service `Foo` becomes a
`FooGrpc` class with a blocking, a future and an async stub; list what is available with:

```bash
ls src/main/java/ondewo/t2s/*Grpc.java
```
...
```
.
├── ondewo-t2s-api/               <----- submodule: the .proto definitions (input)
...
1. `checkout_defined_submodule_versions` — checks out the `ondewo-t2s-api` and
...
```bash
docker run --rm \
  -v $(pwd)/ondewo-t2s-api:/input-volume/ondewo-t2s-api \
  -v $(pwd)/LICENSE:/input-volume/LICENSE \
  -v $(pwd):/output-volume \
  ondewo-java-proto-compiler \
  ondewo-t2s-api ondewo com.ondewo ondewo-t2s-client-java <version>
```
...
* **The output volume is the repository root**, which is precisely the maven project the image
  ...
  is the descriptor being re-rendered from `ONDEWO_T2S_VERSION`, and is intended.
...
`src/main/java/com/ondewo/t2s/auth/`). Two rules follow from the pipeline:
...
Versions are defined once, in the `Makefile`: `ONDEWO_T2S_VERSION` must match the
`ondewo-t2s-api` release in major and minor version, and is written into the generated `pom.xml` by
the next `make build`.

1. Bump `ONDEWO_T2S_VERSION` and the submodule pins in the `Makefile`.
